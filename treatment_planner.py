#!/usr/bin/env python3
"""
Treatment Planner (Prototype)
=============================
Features
--------
  1. Load CT DICOM series → 3-view navigation (axial / sagittal / coronal)
  2. Load dose distribution (DICOM RT Dose, NumPy array, or TOPAS output)
     and overlay it on all three views with a shared colorbar.
  3. Organ / ROI contouring on the axial view with automatic intersection
     lines shown on sagittal and coronal views.
  4. DVH computation displayed in the fourth panel, with statistics.
  5. Save / load contours (JSON).
  6. TOPAS integration: load binary dose output or run a TOPAS macro.

Coordinate system: CT volume [Z, Y, X].  Z index 0 = most-inferior slice.

Usage
-----
    python3 treatment_planner.py
"""
...
BG      = "#1e1e1e"   # main window / sidebar background
BG2     = "#252526"   # panel / toolbar background
BG3     = "#2d2d30"   # inputs, listbox, inactive areas
FG      = "#d4d4d4"   # primary text
FG_DIM  = "#858585"   # secondary / dimmed text
ACCENT  = "#007acc"   # Eclipse / VS Code blue
ACCENT2 = "#4ec9b0"   # teal highlight (Eclipse structure list colour)
BORDER  = "#3e3e42"   # borders, separators
SEL     = "#094771"   # selection highlight
XHAIR   = "#00e5ff"   # bright cyan crosshairs (Eclipse style)
AX_BG   = "#0d0d0d"   # matplotlib axes background
...
def load_ct_series(folder, progress_cb=None):
    """Load CT DICOM series → (volume [Z,Y,X] float32 HU, meta dict).

    Single-pass: reads each file once, skips non-CT files.
    Searches folder and one level of subdirectories.
    """
    ...
    # Stack pixel data (already in memory from single read)
    n_sl = len(slice_info)
    arrays = []
    for i, (_, ds) in enumerate(slice_info):
        if progress_cb:
...
def load_topas_dose(header_path):
    """
    Load TOPAS binary dose output.
    Expects a .header text file and a companion .bin (or same stem) binary.
    ...
    dx = float(params.get("Voxel size X (cm)", 0.1)) * 10.0   # → mm
    ...
    if not os.path.exists(bin_path):
        bin_path = stem + ".binheader"  # TOPAS alt extension
    if not os.path.exists(bin_path):
        raise FileNotFoundError(f"Cannot find binary file for {header_path}")

    raw = np.fromfile(bin_path, dtype=np.float32)
    vol = raw.reshape((nz, ny, nx))   # TOPAS: Z outer, Y middle, X inner
...
def polygon_x_intersections(pts, x_mm):
    """Y-coords where vertical line x=x_mm crosses polygon (mm coords)."""
    result = []
    n = len(pts)
    for i in range(n):
...
def polygon_y_intersections(pts, y_mm):
    """X-coords where horizontal line y=y_mm crosses polygon (mm coords)."""
    result = []
    n = len(pts)
    for i in range(n):
        x1, y1 = pts[i]; x2, y2 = pts[(i + 1) % n]
...
def fill_polygon_mask(pts_mm, ny, nx, row_mm, col_mm):
    """Bool mask [ny, nx] for a polygon defined in mm coordinates."""
    if len(pts_mm) < 3:
...
def compute_dvh(dose_vol, contours, ct_meta):
    ...
    Parameters
    ----------
    dose_vol  : ndarray [Z, Y, X] Gy, on CT grid
    contours  : dict  {z_idx: [ [list of (x_mm, y_mm)], ... ]}
    ct_meta   : CT meta dict

    Returns
    -------
    doses_sorted, vol_pct  (both 1-D arrays for plotting)
    stats dict {Dmin, Dmean, Dmax, D95, D50, vol_cc}
    """
    ...
    stats = {
        ...
        "Dmax":   float(np.max(d)),
        "D95":    float(np.percentile(d, 5)),    # 95% of volume receives ≥ this
...
class TreatmentPlanner:
    def __init__(self, master):
        self.master = master
        master.title("Treatment Planner")
        ...
        # ── Dose ──────────────────────────────────────────────────────
        self.dose_volume   = None        # [Z,Y,X] Gy on CT grid
        ...
        # ── Structures ────────────────────────────────────────────────
        # name → {"color": hex, "contours": {z_idx: [[pts], ...]}}
        self.structures  = {}
        self.struct_order = []
        ...
        self._dvh_cache   = {}   # name -> (doses, vols, stats); cleared on dose/contour change
        self._build_gui()
    ...
    def _load_ct(self):
        ...
        def _worker():
            try:
                vol, meta = load_ct_series(folder, progress_cb=_progress)
                self.master.after(0, lambda: self._on_ct_loaded(vol, meta, folder))
    ...
    def _on_ct_loaded(self, vol, meta, folder):
        self.ct_volume = vol
        self.ct_meta   = meta
        nz, ny, nx = vol.shape
    ...
    def _load_dose_npy(self):
        ...
        if vol.ndim != 3:
            ...
            return
        if self.ct_meta is not None:
            dmeta = self.ct_meta.copy()
        else:
            dmeta = dict(pixel_spacing=[1.0, 1.0], slice_thickness=1.0, image_position=[0, 0, 0])
        self._set_dose(vol, dmeta, os.path.basename(path))
    ...
    def _set_dose(self, vol, dmeta, label):
        ...
        # Navigate to the slice with peak dose so overlay is immediately visible
        if dmax > 0:
    ...
    def _dose_rgba(self, dose_2d):
        ...
        rgba = np.array(plt.cm.jet(norm), dtype=np.float32)  # writable copy
    ...
    def _update_display(self):
        ...
        # ── Apply Eclipse dark theme to all axes ───────────────────
        self.fig.patch.set_facecolor(BG)
        ...
        # Contour intersections on sagittal: find where x=cur_x crosses each polygon
        ...
        # Contour intersections on coronal: find where y=cur_y crosses each polygon
    ...
    def _draw_dvh_panel(self):
        """Draw DVH curves (or a placeholder) in ax_dvh."""
        ax = self.ax_dvh
    ...
    def _pick_topas_macro(self):
        path = filedialog.askopenfilename(
            title="Select TOPAS macro", filetypes=[("Text", "*.txt"), ("All", "*.*")])
        if path:
    ...
    def _run_topas(self):
        if not self._topas_macro:
            ...
            return
        topas_exe = "topas"   # assumes 'topas' is on PATH
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                # Auto-detect output dose header next to macro
